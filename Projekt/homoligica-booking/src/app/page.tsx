...
const HomePage = () => {
  ...
  const openBookingModal = async (service: any) => {
    if (!user) {
      toast.info(
        <div>
          <span>
            Du måste vara inloggad för att boka en tid.
            <p />
          </span>
          <button
            onClick={() => {
  ...
  const handlePaymentChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    setPaymentMethod(event.target.value); // Uppdaterar betalmetod baserat på användarens val
  };
  const bookSlot = async () => {
    ...
    router.push({
      pathname:
        paymentMethod === "credit_card" ? "/payment/card" : "/payment/swish",
      query: {
        selectedService: JSON.stringify(selectedService), // Skicka som JSON-sträng
        selectedTime,
        paymentMethod,
      },
    });
    ...
    if (slotError) {
      ...
      toast.warning(
        "Bokningen registrerades men tiden kunde inte markeras som bokad."
      );
    } else {
  ...
  const handleNotifyMe = async () => {
    ...
    if (error || !user) {
      alert("Du måste vara inloggad för att få en notis.");
      return;
    }

    // Hämta e-post från 'customers' tabellen baserat på användarens UUID
    const { data: userInfo, error: userFetchError } = await supabase
      .from("customers")
      .select("email")
      .eq("id", user.id)
    ...
    if (userFetchError || !userInfo) {
      alert("Kunde inte hämta din e-postadress.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/notify", {
        ...
        body: JSON.stringify({
          ...
          service: selectedService.name, // Skickar tjänstens namn här
        }),
      });

      if (response.ok) {
        toast.success("Du kommer få ett mejl när en tid blir ledig!");
      } else {
        const errorData = await response.json();
        console.error("API-fel:", errorData.message);
        toast.warning("Något gick fel. Försök igen senare.");
      }
    } catch (error) {
      console.error("Fel vid e-postanmälan:", error);
      toast.warning("Något gick fel. Försök igen senare.");
    }
  };
    ...
    <div className="min-h-screen bg-gray-100">
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-8 flex-1">
              <div className="flex flex-col items-center md:items-start">
                ...
                <p className="text-gray-600 text-lg mb-4 text-center md:text-left">
                  Välkommen till Homologicas on-line-bokning. Är du tveksam vad
                  du ska boka så kan du få personlig service måndag, tisdag och
                  torsdagar mellan kl 10-18 på telefonnummer 029021020.
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 mb-8 flex-1 border border-gray-200">
              ...
              {sortedOpenHours.length > 0 ? (
                sortedOpenHours.map((hour) => (
                  <p
                    key={hour.id}
                    className="text-gray-600 hover:text-blue-600 transition duration-300 ease-in-out mb-4"
                  >
      ...
      {selectedService && (
        ...
        >
          ...
          <div className="text-center mb-4">
            <p className="text-blue-600 font-bold mt-2">
              Pris: {selectedService.price} kr
            </p>
          </div>

          {/* Filtrera bort tider i det förflutna */}
          {availableSlots.filter((slot) => new Date(slot.date) >= new Date())
            .length > 0 ? (
            <>
              <div className="mb-4">
                ...
                >
                  ...
                  {availableSlots
                    .filter((slot) => new Date(slot.date) >= new Date()) // Tar bort förflutna tider
                    .map((slot) => {
              ...
              {/* Betalningsalternativ visas endast om det finns lediga tider */}
              <div className="mb-4">
                <p className="text-center text-black font-semibold mt-4">
                  Välj betalning
                </p>
                ...
                >
                  ...
                  <option value="credit_card" className="text-gray-700">
                    Kortbetalning
                  </option>
                  <option value="swish" className="text-gray-700">
                    Swish
                  </option>
                </select>
              </div>
              ...
              <div className="flex justify-between mt-4">
                ...
                <button
                  ...
                  onClick={bookSlot}
                  disabled={!paymentMethod || !selectedTime} // Knappen är inaktiv om ingen tid eller betalmetod är vald
                >
                  Boka Tid
                </button>
              </div>
            ...
            <>
              <p className="text-center text-red-600 font-semibold">
                Tyvärr, inga lediga tider just nu.
              </p>
              <div className="text-center mt-4">
                ...
                >
                  Meddela mig när en tid blir ledig
                </button>
              </div>
            </>
          )}
        ...
        <form
          onSubmit={async (e) => {
            ...
            try {
              ...
              if (userInsertError) {
                toast.error(
                  "Fel vid sparning av användaruppgifter: " +
      ...
      <ToastContainer
        ...
        style={{
          ...
          fontSize: "18px", // Öka textstorleken för att göra toasten större
          padding: "20px", // Lägg till padding för att öka storleken på själva toasten
          width: "px", // Justera bredden för toasten
        }}
      />
    </div>
  );
